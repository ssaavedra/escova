...
case class NodeCostConfig(
    ...
    /** If non-empty, only these type of children will be allowed as subaggregates.
      */
    allowlistChildren: List[String] = List(),

    /** If non-empty these types of children will be disallowed as subaggregates.
      */
    blocklistChildren: List[String] = List(),
) {
    /** Determine whether a TreeNode fits the allow/block lists.
      *
      * Take care also of the max height allowed.
      *
      * @param t the TreeNode to include
      * @return whether the children is allowed at the current point
      */
    def isChildAllowed(t: TreeNode): Boolean = {
        val allowlisted =
            if (allowlistChildren.nonEmpty)
                allowlistChildren.contains(t.node.getType)
            else
                !blocklistChildren.contains(t.node.getType)

        allowlisted && t.height < maxTreeHeight
    }
}
